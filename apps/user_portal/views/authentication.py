...
from apps.user_portal.serializers.authentication import (
    SendPasswordResetByEmailSerializer,
    UserPasswordResetByEmailVerificationSerializer,
    SendPasswordResetByOTPSerializer,
    UserPasswordResetByOTPVerificationSerializer,
    UserAccountActivationByOTPVerificationSerializer,
...
class SendPasswordResetByOTPViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request: Request) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        return Response(
            {"message": "OTP has been send. Please check your Phone"},
            status=status.HTTP_200_OK,
        )
...
class UserPasswordResetByOTPVerificationViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request: Request) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        return Response(
            {"message": "OTP Verification Complete. Password is reset."},
            status=status.HTTP_200_OK,
        )
...
class UserAccountActivationByOTPVerificationViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request: Request) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        return Response(
            {"message": "Account verified by OTP"}, status=status.HTTP_200_OK
        )
...
class UserAccountActivationByEmailVerificationViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request: Request, uid: str, token: str) -> Response:
        ...
        if not user.is_active:
            ...
            user.save()
        return Response(
            {"message": "Account Activation Successfully"}, status=status.HTTP_200_OK
        )
